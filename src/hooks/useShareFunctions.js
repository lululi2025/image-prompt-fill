 ...
 * @param {Function} setActiveTemplateId - 設定激活模板的函数
 * @param {Function} setDiscoveryView - 設定發現页視圖的函数
 * @param {boolean} isMobileDevice - 是否为移动设备
 * @param {Function} setMobileTab - 設定行動端標籤的函数
 * @param {string} language - 当前语言
 * @param {Function} t - 翻译函数
 * @param {Object} banks - 詞庫数据
 * @param {Function} setBanks - 更新詞庫的函数
 * @param {Object} categories - 分類数据
 * @param {Function} setCategories - 更新分類的函数
 * @param {Array} templates - 所有模板数据（用于打包关联模板）
...
export const useShareFunctions = (
  ...
  const [isGenerating, setIsGenerating] = useState(false); // 新增：网络請求状态
  const [prefetchedShortCode, setPrefetchedShortCode] = useState(null); // 新增：預取的短码
  ...
  // 计算分享 URL（保留作为兜底的長連結預覽，但在实际分享时会尝试生成短链）
  ...
  useEffect(() => {
    const handleCheckShare = async () => {
      ...
      if (shareData) {
        ...
        // --- 核心改动：支持短码解析 ---
        // 如果 shareData 长度较短（比如 8 位），说明是短码，需要去后端請求
  ...
  const handleImportSharedTemplate = useCallback(() => {
    ...
    // 清除 linkedTemplates 字段（不需要存储到本地模板中）
    delete templateToImport.linkedTemplates;

    // --- 0.5 更新主模板 source 中的 templateId 为新 ID ---
    ...
    sortedOldKeys.forEach(oldKey => {
      ...
      if (banks[oldKey]) {
        // 如果本地已存在同名键，且內容不完全一致，则需要重命名匯入的詞庫
    ...
    if (Object.keys(keyMap).length > 0) {
      ...
      Object.entries(templateToImport.selections).forEach(([selKey, val]) => {
        ...
        sortedOldKeys.forEach(oldKey => {
          if (keyMap[oldKey]) {
            const newKey = keyMap[oldKey];
            // 匹配开头是 oldKey，后面跟着 - (普通變數) 或 _ (聯動組)
  ...
  const handleShareLink = useCallback(() => {
    ...
    if (activeTemplate && !prefetchedShortCode && !isPrefetching) {
      ...
      getShortCodeFromServer(compressed).then(code => {
        ...
        if (!code) setShortCodeError(language === 'cn' ? '短連結生成失败，已回退为長連結' : 'Short link generation failed; using long link');
      }).catch(() => {
        setShortCodeError(language === 'cn' ? '短連結生成失败，已回退为長連結' : 'Short link generation failed; using long link');
      }).finally(() => {
        setIsPrefetching(false);
      });
    }
  ...
  const doCopyShareLink = useCallback(async () => {
    ...
    try {
      ...
      if (success) {
        ...
        alert(language === 'cn' ? '複製失败，請手動複製下方連結' : 'Copy failed, please manually copy the link below');
      }
    } catch (err) {
  ...
  const handleShareToken = useCallback(async () => {
    ...
    try {
      ...
      const tokenText = `「Prompt分享」我的新模板：${templateName}\n複製整段文字，打开【提示詞填空器】即可匯入：\n#pf$${finalToken}$`;

      const success = await copyToClipboard(tokenText);
      if (success) {
        alert(language === 'cn' ? '分享口令已複製，快去发给好友吧！' : 'Share token copied!');
        setShowShareOptionsModal(false);
      }
    } finally {
      setIsGenerating(false);
  ...
  // 计算分享 URL（优先显示短码連結，失败则显示長連結作为兜底）
  ...
  return {
    ...
    importTokenValue,
    shareUrlMemo,
    currentShareUrl, // 新增：当前生成的分享連結
    isGenerating,
    isPrefetching, // 新增：暴露預取状态
    prefetchedShortCode, // 新增：暴露預取结果
